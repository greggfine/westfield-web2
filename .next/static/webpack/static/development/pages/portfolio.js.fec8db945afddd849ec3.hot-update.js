webpackHotUpdate("static/development/pages/portfolio.js",{

/***/ "./node_modules/css-loader/dist/cjs.js?!./node_modules/postcss-loader/src/index.js?!./node_modules/resolve-url-loader/index.js?!./node_modules/sass-loader/dist/cjs.js?!./pages/portfolio.module.scss":
/*!**************************************************************************************************************************************************************************************************************************************************************!*\
  !*** ./node_modules/css-loader/dist/cjs.js??ref--5-oneOf-3-1!./node_modules/postcss-loader/src??__nextjs_postcss!./node_modules/resolve-url-loader??ref--5-oneOf-3-3!./node_modules/sass-loader/dist/cjs.js??ref--5-oneOf-3-4!./pages/portfolio.module.scss ***!
  \**************************************************************************************************************************************************************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

exports = module.exports = __webpack_require__(/*! ../node_modules/css-loader/dist/runtime/api.js */ "./node_modules/css-loader/dist/runtime/api.js")(true);
// Module
exports.push([module.i, ".portfolio_Portfolio__websites__2rPvN {\n  list-style-type: none;\n  display: flex;\n  flex-wrap: wrap;\n  justify-content: space-around;\n  margin: 60px auto;\n  padding: 40px 0px;\n  font-size: 1.5rem;\n  width: 60%; }\n\n.portfolio_Portfolio__heading__3RS8x {\n  border: 1px solid #000;\n  background-color: #1f062c;\n  background-image: url(\"/domenico-loia-hGV2TfOh0ns-unsplash.jpg\");\n  background-repeat: no-repeat;\n  background-size: cover;\n  background-position: center;\n  background-blend-mode: screen;\n  -webkit-filter: grayscale(40%);\n          filter: grayscale(40%);\n  width: 100vw;\n  height: 400px;\n  display: flex;\n  align-items: center;\n  justify-content: center; }\n\n.portfolio_Portfolio__title__dcCcs {\n  color: #fff;\n  font-weight: 700;\n  font-size: 4rem;\n  text-outline: 1px solid red;\n  -webkit-text-stroke-width: 1px;\n  -webkit-text-stroke-color: black; }\n\n.portfolio_Portfolio__subheading__1pb3A {\n  color: #fff;\n  -webkit-text-stroke-width: 0.5px;\n  -webkit-text-stroke-color: black; }\n\n.portfolio_Portfolio__Portfolio__headerWrapper__1QHfV {\n  background-color: #fff;\n  border: 10px solid #000; }\n", "",{"version":3,"sources":["portfolio.module.scss"],"names":[],"mappings":"AACE;EACE,qBAAqB;EACrB,aAAa;EACb,eAAe;EACf,6BAA6B;EAC7B,iBAAiB;EACjB,iBAAiB;EACjB,iBAAiB;EACjB,UAAU,EAAA;;AAGZ;EACE,sBAAsB;EAEtB,yBAAyB;EACzB,gEAAgE;EAChE,4BAA4B;EAC5B,sBAAsB;EACtB,2BAA2B;EAC3B,6BAA6B;EAC7B,8BAAsB;UAAtB,sBAAsB;EACtB,YAAY;EACZ,aAAa;EACb,aAAa;EACb,mBAAmB;EACnB,uBAAuB,EAAA;;AAGzB;EACE,WAAW;EACX,gBAAgB;EAChB,eAAe;EACf,2BAA2B;EAC3B,8BAA8B;EAC9B,gCAAgC,EAAA;;AAGlC;EACE,WAAW;EACX,gCAAgC;EAChC,gCAAgC,EAAA;;AAGlC;EACE,sBAAsB;EACtB,uBAAuB,EAAA","file":"portfolio.module.scss","sourcesContent":[".Portfolio {\n  &__websites {\n    list-style-type: none;\n    display: flex;\n    flex-wrap: wrap;\n    justify-content: space-around;\n    margin: 60px auto;\n    padding: 40px 0px;\n    font-size: 1.5rem;\n    width: 60%;\n  }\n\n  &__heading {\n    border: 1px solid #000;\n    // background-color: #4a1962;\n    background-color: #1f062c;\n    background-image: url(\"/domenico-loia-hGV2TfOh0ns-unsplash.jpg\");\n    background-repeat: no-repeat;\n    background-size: cover;\n    background-position: center;\n    background-blend-mode: screen;\n    filter: grayscale(40%);\n    width: 100vw;\n    height: 400px;\n    display: flex;\n    align-items: center;\n    justify-content: center;\n  }\n\n  &__title {\n    color: #fff;\n    font-weight: 700;\n    font-size: 4rem;\n    text-outline: 1px solid red;\n    -webkit-text-stroke-width: 1px;\n    -webkit-text-stroke-color: black;\n  }\n\n  &__subheading {\n    color: #fff;\n    -webkit-text-stroke-width: 0.5px;\n    -webkit-text-stroke-color: black;\n  }\n\n  &__Portfolio__headerWrapper {\n    background-color: #fff;\n    border: 10px solid #000;\n  }\n}\n"]}]);
// Exports
exports.locals = {
	"Portfolio__websites": "portfolio_Portfolio__websites__2rPvN",
	"Portfolio__heading": "portfolio_Portfolio__heading__3RS8x",
	"Portfolio__title": "portfolio_Portfolio__title__dcCcs",
	"Portfolio__subheading": "portfolio_Portfolio__subheading__1pb3A",
	"Portfolio__Portfolio__headerWrapper": "portfolio_Portfolio__Portfolio__headerWrapper__1QHfV"
};

/***/ })

})
//# sourceMappingURL=portfolio.js.fec8db945afddd849ec3.hot-update.js.map